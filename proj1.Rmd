...
```{r}
graph3=ggplot(, aes(x=df4_new_pca$x[,1], y=hamatop, col=Height)) +
  geom_line(aes(group=Height))+scale_color_gradient(low = "blue", high = "yellow")
```
...
```{r}
fviz_nbclust(df4_new_pca$x[,1:2], kmeans, method = 'wss')
...
library(cluster)
library(factoextra)
km.res <- kmeans(df4_new_pca$x[,1:2], centers=3, nstart = 25)
fviz_cluster(km.res, data = df4_new_pca$x[,1:2])
```
