...
## (e)
When we are combining the datasets from both net and log,We find that there are some duplicated rows which should be deleted. Also, after reading the paper, we realize that data from node with voltage higher than 3.0 or lower than 2.4 should be removed, but we should keep those data from network with voltage all larger than 200.
```{r}
df4 <- df4 %>%
...
graph1
```
We plotted the relationship between first two PCs with PC1 as x axis and PC2 as y axis, and we color our points with the value of heights. We find that there is an obvious stratification for our points based on the value of height, which is interesting. Points with smaller values of heights gather closely on the top of the graph, while points with larger values of heights  gather on the on the bottom of the graph. This might imply that there is a strong correlation between height and second PC.
...
## Figure 3[a]
In Figure 3[a], we notice the histogram of "Incident PAR" and "Reflected PAR". Both these two graph have a long tail of small-value or zero-value readings. Comparing to the large-value reading at the right of these two histograms, it would be very hard for readers to check the value of readings in the tail parts of these two histogram. In this case, I would recommend to adjust the data scale and apply an log-transformation on data. After the log-transformation, we could remove the zero-value from plots and shrink the value-difference between large-value reading and small-value reading. In this way, we could have a better visualization on the whole plot, especially on the tail part of the histogram of "Incident PAR" and "Reflected PAR".
```{r}
hist(log(df4_new$hamatop))
hist(log(df4_new$hamabot))
```
...
From our opinion, Tolle et al. could calculate the variance at different height group and draw the line graph to check the distribution of all four variables vary at different heights.
```{r}
...
plot(x$Height,x$humidity_var,type = "l")
plot(x$Height,x$temp_var,type = "l")
plot(x$Height,x$hamatop_var,type = "l")
plot(x$Height,x$hamabot_var,type = "l")
```

## Figure4
For figure 4, it's very hard to distinguish all the colors. It would be better for Tolle et al. to add the legend so that reader could understand the corresponding height of each color line. Moreover, the first two plots on the left part included. If Tolle et al. could cut the whole day time period to several smaller time period, such as 5:00:00am- 10:00:00am (sunrise period), 10:00:00am-16:00:00, 16:00:00-21:00:00 (sunset period), and 21:00:00-5:00:00am. The visualization effect could be better. 
